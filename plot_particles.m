...
setfield(particle_list(1),'p',5);
for j=1:length(particle_list)
    test_ball = particle_list(j);
    color = particle_list(j).color;
    p = plot3(ax,real(test_ball.x(1,2)+test_ball.x(2,1))/2, imag(test_ball.x(2,1)-test_ball.x(1,2))/2, real(test_ball.x(1,1)-test_ball.x(2,2))/2, '.', 'Color', color, 'markersize', 50);
%     particle_list(j) = setfield(particle_list(j),'p',p);
